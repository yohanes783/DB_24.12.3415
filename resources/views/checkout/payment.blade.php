@extends('layouts.app')
@section('title', 'Pembayaran - ' . ($transaction->event->title ?? 'Tiket'))
...
<main class="max-w-3xl mx-auto px-6 py-20 text-center">
    <div class="bg-white rounded-3xl border border-slate-200 p-12 shadow-sm inline-block w-full max-w-md">
        ...
        <p class="text-slate-500 mb-8">Mohon selesaikan pembayaran tiket Anda untuk event <strong>{{ $transaction->event->title ?? 'Event' }}</strong>.</p>
...
<!-- PERBAIKAN 1: Gunakan env() langsung atau fallback agar Client Key tidak null -->
...
<script type="text/javascript">
    ...
    payButton.onclick = function () {
        // PERBAIKAN 2: Pastikan snap_token tersedia sebelum memanggil Snap
        const snapToken = '{{ $transaction->snap_token }}';

        if (!snapToken) {
            alert("Token pembayaran tidak ditemukan. Silakan coba checkout ulang.");
            return;
        }
    ...
    // PERBAIKAN 3: Delay otomatis klik agar Midtrans JS selesai dimuat sempurna oleh browser
