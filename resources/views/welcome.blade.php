...
<body class="bg-slate-50 text-slate-900">
    ...
    <section class="max-w-7xl mx-auto px-6 py-20 flex flex-col md:flex-row items-center gap-12">
        ...
        <div class="flex-1 relative">
            ...
            <!-- PERBAIKAN: Pembungkus Gambar Agar Ukurannya Proporsional Layaknya Banner -->
            <div class="w-full aspect-[16/9] md:aspect-[4/3] rounded-[2.5rem] overflow-hidden shadow-2xl border-8 border-white bg-slate-100">
                @if($events->count() > 0)
                    <img src="{{ ($events->first()->poster_path && Storage::disk('public')->exists($events->first()->poster_path))
                             ? asset('storage/' . $events->first()->poster_path)
                             : 'https://placehold.co' }}" 
                         alt="{{ $events->first()->title }}"
                         class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                @else
                    <!-- Mengubah resolusi placeholder cadangan dari 200x600 ke bentuk landscape 800x450 -->
    ...
    <!-- === [TAMBAHAN SOAL 4]: SECTION PARTNER & SPONSOR PUBLIC SCREEN === -->
    <section class="border-t border-slate-100 bg-white py-16">
        <div class="max-w-7xl mx-auto px-6 text-center">

            <h2 class="text-xs font-bold uppercase tracking-widest text-indigo-500 mb-2">Didukung Oleh</h2>
            <p class="text-2xl font-black text-slate-800 mb-10">Partner Resmi AmikomEventHub</p>

            <!-- Grid Perulangan Blade Engine -->
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 justify-center items-center">
                @forelse($partners as $partner)
                    <div class="p-4 bg-slate-50/60 border border-slate-100 rounded-2xl flex items-center justify-center h-20 shadow-sm transition hover:shadow-md hover:-translate-y-0.5 transform duration-200">
                        <img src="{{ $partner->logo_url }}"
                             alt="{{ $partner->name }}"
                             title="{{ $partner->name }}"
                             class="max-w-full max-h-full object-contain grayscale opacity-60 hover:grayscale-0 hover:opacity-100 transition duration-300">
                    </div>
                @empty
                    <p class="text-sm text-slate-400 font-medium col-span-full py-4">Belum ada partner resmi yang terdaftar.</p>
                @endforelse
            </div>
