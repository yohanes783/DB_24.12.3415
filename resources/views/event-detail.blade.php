...
<main class="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div class="lg:col-span-1">
            <div class="sticky top-32">
                @php
                    $posterSrc = 'https://placehold.co/200x600';
                    if ($event->poster_path) {
                        if (\Illuminate\Support\Str::startsWith($event->poster_path, 'http')) {
                            $posterSrc = $event->poster_path;
                        } elseif (\Illuminate\Support\Facades\Storage::disk('public')->exists($event->poster_path)) {
                            $posterSrc = asset('storage/' . $event->poster_path);
                        }
                    }
                @endphp
                <img src="{{ $posterSrc }}" alt="{{ $event->title }}" class="w-full rounded-[2.5rem] shadow-2xl border-8 border-white object-cover aspect-[3/4]">
